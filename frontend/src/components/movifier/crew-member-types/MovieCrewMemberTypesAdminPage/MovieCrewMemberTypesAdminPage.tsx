...
import { MovieCrewMemberTypeCard } from '../CrewMemberTypeCard/CrewMemberTypeCard'
import {
      ...
      <header className='sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6'>
        <Breadcrumb className='hidden md:flex'>
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                <Link href='#'>Crew member types</Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
          ...
          <div className='grid'>
            <Card className='sm:col-span-2'>
              <CardHeader className='pb-3 w-full'>
                <CardTitle>Crew member types</CardTitle>
                <CardDescription className='max-w-lg text-balance leading-relaxed'>
                  Manage movie crew member types here
                </CardDescription>
              </CardHeader>
            </Card>
          </div>
            ...
            <CardContent className={'h-full relative'}>
              <div className='grid grid-cols-3 justify-center gap-5 m-5'>
                {data?.movieCrewMemberTypes
                  ...
                  .map((type) => (
                    <MovieCrewMemberTypeCard key={type.id} {...type} />
                  ))}
              </div>

              <Link href={'/movies/crew-member-types/new'} passHref>
                <motion.div
