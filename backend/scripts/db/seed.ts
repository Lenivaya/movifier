...
async function main() {
  await prisma.$transaction(async (tx) => {
    await tx.movifierAppUser.create({
      data: {
    ...
    // CrewMemberTypes
    await tx.movieCrewMemberType.createMany({
      data: [
        { name: "Director" },
        { name: "Producer" },
        { name: "Writer" },
    ...
    // MovieStudios
    await tx.movieStudio.createMany({
      data: [
    ...
    const david_fincher = await tx.movieCrewMember.create({
      data: {
        name: "David Fincher",
        photoUrl:
    ...
    const michael_fassbender = await tx.movieCrewMember.create({
      data: {
    ...
    const tilda_swinton = await tx.movieCrewMember.create({
      data: {
        name: "Tilda Swinton",
        photoUrl:
    ...
    const the_killer_2023 = await tx.movie.create({
      data: {
        movieInfo: {
          create: {
            imdbId: "tt1136617",
    ...
    // Pulp fiction 1994
    const quentin_tarantino = await tx.movieCrewMember.create({
      data: {
    ...
    const john_travolta = await tx.movieCrewMember.create({
      data: {
        name: "John Travolta",
        photoUrl:
    ...
    const bruce_willis = await tx.movieCrewMember.create({
      data: {
        name: "Bruce Willis",
        photoUrl:
    ...
    const harvey_keitel = await tx.movieCrewMember.create({
      data: {
        name: "Harvey Keitel",
        photoUrl:
    ...
    const pulp_fiction = await tx.movie.create({
      data: {
        movieInfo: {
          create: {
            imdbId: "tt0110912",
